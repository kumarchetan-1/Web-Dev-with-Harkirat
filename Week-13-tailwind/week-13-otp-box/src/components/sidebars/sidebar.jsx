...
export function Sidebar() {
  const toggleSidebar = useRecoilValue(toggleSidebarAtom)
  const setSidebarToggleState = useSetRecoilState(toggleSidebarAtom)
    ...
    if (isMobileScreen) {
      setMobSidebar(<aside className="h-8 w-8 absolute left-0 top-0">
        <div className="flex flex-nowrap justify-between align-middle gap-2">
          <SidebarToggle setSidebarToggle={setSidebarToggleState} toggleSidebar={toggleSidebar} />
          {profilePhotoElem()}
        </div>

      </aside>)
    } else {
      setMobSidebar(<aside className="h-screen p-6 border border-r-2 border-y-0 border-l-0  min-w-20 bg-white">
        <div className="flex flex-nowrap justify-between align-middle gap-2">
          <SidebarToggle setSidebarToggle={setSidebarToggleState} toggleSidebar={toggleSidebar} />
          {profilePhotoElem()}
        </div>
  ...
  const expendedSidebar = () => <aside className={`h-screen overflow-auto p-6 border border-r-2 border-y-0 border-l-0 w-80 bg-white ${isMobileScreen && "absolute left-0 top-0 z-10"}`}>
    <div className="flex flex-nowrap justify-between align-middle gap-2">
      <SidebarToggle setSidebarToggle={setSidebarToggleState} toggleSidebar={toggleSidebar} />
      {profilePhotoElem()}
    </div>
